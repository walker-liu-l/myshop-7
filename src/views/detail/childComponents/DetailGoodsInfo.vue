<template>
  <div id="detail_goods_info" v-if="detailInfo.length != 0">
    ...
    <div class="goods_info_key">{{ detailInfo.detailImage[0] }}</div>
    <div class="goods_info_imgs">
      <img :src="item" v-for="(item, index) in detailInfo.detailImage[1]" :key="index" @load="loadImg"/>
    </div>
  </div>
</template>
<script>
